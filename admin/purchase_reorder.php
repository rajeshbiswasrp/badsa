...
$mrp='';
$ptr='';
$base_type=$_REQUEST['base_type'.$value];;
//$type_id=$_POST["type_id"];
$t_spq=$_REQUEST['reorderlevel'.$value];
$nts=$_REQUEST['nts'.$value];;
$free_qty='';
$tax_type='';
$tax_per='';
$ed_per='';
...
{
?>
<script>window.location="purchase_order.php"</script>
<?php
}
?>
...
<script language="javascript">
	...
	{
		$.ajax({ type: "POST",url: "ajax_check_po.php",async:false,success: function(returndata)
		{
			if(returndata!='')
			{
					if(parseInt(returndata)==1)
					{
						...
						alert('Please clear purchase order first then reorder here.');
						flag=0;
...
var cloned = obj.parentNode.parentNode;
var obj1 = document.getElementById('listtable');
obj1.appendChild(cloned);
...
<script>
function showchange(btypevalue,idvalue){
if(btypevalue==1)
document.getElementById("nts"+idvalue).style.display = "none";	
else
document.getElementById("nts"+idvalue).style.display = "block";	
return ;
}
</script>
...
<!--*********************************-->
		<div class="span12">		
			...
			<h4 style="margin-top:0px; font-size:15px;">Purchase Reorder <small class="pull-right">&nbsp;</small></h4>
...
?>
     ...
     <?php /*?> <option value="<?php echo $rown3['id']?>"><?php echo $rown3['sup_name']?></option><?php */?>
...
?>
	...
	<div id="" style="padding:0px; width:100%; height:; border-bottom:solid 1px #ccc;">
  ...
  <tr>
    ...
    <td style="background-color:#009900; color:#fff; padding:2px 4px;"><div align="center"><strong>Last Date of Purchase</strong></div></td>
...
<h5 style="margin-top:0px; font-size:15px;">Reorder Medicine List <small class="pull-right">&nbsp;</small></h4>		
	<table class="table table-hover table-striped table-bordered" width="100%" border="0" cellpadding="12" cellspacing="8" style="margin-top:3px; font-size:12px; text-align:center;" id="listtable">
  <tr>
...
while($rowi = mysql_fetch_array($resulti))
{
	...
	$issue_quantity=$issue_quantity+$rowsmaster['iss_qty'];
	}
	...
	$sqlsreturn="SELECT ph_sales_return.* from ph_sales_return left join ph_sales_master on ph_sales_return.ph_sales_id=ph_sales_master.id where ph_sales_master.purchase_id='$purchase_id' and ph_sales_return.date <= '$date1'";
...
if($value<$row['reorder_level']){
	
	
$sqllast="SELECT * FROM ph_purchase_master where id=(
    SELECT max(id) FROM ph_purchase_master where medicine_id='$key'
    )";
$resultlast=mysql_query($sqllast);
if(mysql_num_rows($resultlast)>0){
while($rowlast=mysql_fetch_array($resultlast)){
$lastpurchasedate=$rowlast['date'];
}
}
else
$lastpurchasedate='';
?>
  ...
  <tr>
    ...
    <td style="padding:2px 4px;"><div align="center"><strong><input type=text name='dateofpurchase<?php echo $row[id];?>' id='dateofpurchase<?php echo $row[id];?>' value="<?php echo $lastpurchasedate;?>" readonly/></strong></div></td>
    ...
    <td style="padding:2px 4px;"><div align="center"><strong><input type="text" name="reord<?php echo $row[id];?>" value="<?php echo $row["reorder_level"]; ?>" readonly/></strong></div></td>
     <td style="padding:2px 4px;"><div align="center"><strong>
     <select name="base_type<?php echo $row[id];?>" id="base_type<?php echo $row[id];?>"  style=" margin-bottom:0px; width:80px;" onchange="showchange(this.value,'<?php echo $row[id];?>')"> 
     <option value="1">Pieces</option>
     <option value="0">Strip / Box</option>
     </select><input placeholder="nts" type="text" name="nts<?php echo $row[id];?>" id="nts<?php echo $row[id];?>" style="display:none;width:30px; margin-bottom:0px;"/></strong></div></td>
    <td style="padding:2px 4px;"><div align="center"><strong><input style="margin-bottom:0px; width:80px;" type=text name='reorderlevel<?php echo $row[id];?>' id='reorderlevel<?php echo $row[id];?>' value="" required/></strong></div></td>
    <td style="padding:2px 4px;"><input onclick="clonethis(this);" type="checkbox" id="checkbox<?php echo $c;?>" name="allbox2[]" value="<?php echo $row[id];?>" />Reorder&nbsp;&nbsp;<a href='#' onclick="removethis(this);">Delete</a></td>
  </tr>
...
$anotherlist=substr($anotherlist,0,strlen($anotherlist)-1);
?>
<?php
if(strlen($otherlist)>0)
{
   if(strlen($anotherlist)>0)
   {
	   $othersql ="id not in ($otherlist,$anotherlist)";
   }
   else
   $othersql ="id not in ($otherlist)";
}
else
if(strlen($anotherlist)>0)
{
   $othersql ="id not in ($anotherlist)";
}
else
$othersql ="1";

$othersql="select * from ph_medicine_master where $othersql";
...
?>
  ...
  <tr>
    ...
    <td style="padding:2px 4px;"><div align="center"><strong><input type=text name='dateofpurchase<?php echo $rowother[id];?>' id='dateofpurchase<?php echo $row[id];?>' value="<?php echo $lastpurchasedate;?>" readonly/></strong></div></td>
    <td style="padding:2px 4px;"><div align="center"><strong><input type=text name='stockinhand<?php echo $rowother[id];?>' id='stockinhand<?php echo $rowother[id];?>' value="0" readonly/></strong></div></td>
    <td style="padding:2px 4px;"><div align="center"><strong><input type="text" name="reord<?php echo $rowother[id];?>" value="<?php echo $rowother["reorder_level"]; ?>" readonly/></strong></div></td>
     <td style="padding:2px 4px;"><div align="center"><strong>
     <select name="base_type<?php echo $rowother[id];?>" id="base_type<?php echo $rowother[id];?>"  style=" margin-bottom:0px; width:80px;" onchange="showchange(this.value,'<?php echo $rowother[id];?>')"> 
     <option value="1">Pieces</option>
     <option value="0">Strip / Box</option>
     </select><input placeholder="nts" type="text" name="nts<?php echo $rowother[id];?>" id="nts<?php echo $rowother[id];?>" style="display:none;width:30px; margin-bottom:0px;"/></strong></div></td>
    <td style="padding:2px 4px;"><div align="center"><strong><input  style="margin-bottom:0px; width:80px;" type=text name="reorderlevel<?php echo $rowother[id];?>" id="reorderlevel<?php echo $rowother[id];?>" value="" required/></strong></div></td>
    <td style="padding:2px 4px;"><input onclick="clonethis(this);" type="checkbox" id="checkbox<?php echo $c;?>" name="allbox2[]" value="<?php echo $rowother[id];?>" />Reorder&nbsp;&nbsp;<a href='#' onclick="return removethis(this);">Delete</a></td>
  </tr>
