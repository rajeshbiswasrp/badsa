...
<!--*********************************-->
		<div class="span12">
			...
			<h4>All Item Wise Report<small class="pull-right">&nbsp;</small></h4>
            <div align="center"><?php echo $msg;?><?php echo $msg1;?>
	  </div>
			<div class="row-fluid" style="background-color:#fff; padding:10px 10px 10px 10px; font-size:14px; text-align:justify; line-height:25px; width:98%;">
            
			<div id="sidebar" class="span12">
<form  action="item_all_report.php" name="form" id="form" method="post" enctype="multipart/form-data">
<table class="table table-hover table-striped table-bordered">
    <tr>
    <td><strong>Item</strong></td>
    <td  style="padding:3px 5px;">
      <select name="tiem_id" id="tiem_id" style=" margin-bottom:0px;">
        <option value="">--Select--</option>
        <option value="0">All</option>
<?php
$sql22 = "SELECT DISTINCT medicine_id FROM ph_purchase_master";
$result22 = mysql_query($sql22);
while($row22 = mysql_fetch_array($result22)){
$m_id=$row22['medicine_id'];

$sql33 = "SELECT * FROM ph_medicine_master where id='$m_id' ORDER BY medici_name ASC";
$result33 = mysql_query($sql33);
while($row33 = mysql_fetch_array($result33)){
?>
              <option value="<?php echo $row33['id']?>"><?php echo $row33['medici_name']?></option>
<?php
}
}
?>
...
<table class="table table-hover table-striped table-bordered" id="product-table">
      ...
      <th><strong>Stock Value</strong></th>
      <th><strong>Sale Value</strong></th>
    </tr>
        </tbody>
<tbody>           
<?php
...
if($tiem_id=='0')
{
$sql="SELECT * FROM ph_purchase_master";
}
if($tiem_id!='0' && $size_type=='' && $for_type=='')
{
$sql="SELECT * FROM ph_purchase_master where medicine_id='$tiem_id'";
}
if($tiem_id!='' && $size_type!='' && $for_type=='')
{
$sql="SELECT * FROM ph_purchase_master where medicine_id='$tiem_id' and size_type='$size_type'";
}
if($tiem_id!='' && $size_type!='' && $for_type!='')
{
$sql="SELECT * FROM ph_purchase_master where medicine_id='$tiem_id' and size_type='$size_type' and for_type='$for_type'";
}

//$sql="SELECT * FROM ph_purchase_master where medicine_id='$tiem_id' or size_type='$size_type' or for_type='$for_type'";
...
$stock_v=$pptr*$nofp_inhand;
$sale_v=$mmrp*$nofp_inhand;

$sum=$sum+$stock_v;
$sum2=$sum2+$sale_v;
$sum3=$sum3+$nofp_inhand;
?>       
    <tr>
      ...
      <td><?php echo $stock_v; ?></td>
      <td><?php echo $sale_v; ?></td>
      </tr>
<?php 
$c=$c+1;
}
